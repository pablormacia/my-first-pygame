...
class Weapon():
    def __init__(self, image, imagen_bala):
        ...
        # Tomo el tiempo del último disparo
        self.ultimo_disparo = pygame.time.get_ticks()

    def update(self, personaje):
        # Tiempo de espera para poder disparar la siguiente bala
        ...
        if pygame.mouse.get_pressed()[0] and self.disparada == False and pygame.time.get_ticks() - self.ultimo_disparo >= disparo_cooldown:  # clic izq
            bala = Bullet(self.imagen_bala, self.forma.centerx,
                          self.forma.centery, self.angulo)
            self.disparada = True
            self.ultimo_disparo = pygame.time.get_ticks()
...
class Bullet(pygame.sprite.Sprite):  # Hereda de la clase pygame.sprite.Sprite
    def __init__(self, image, x, y, angle):
        # Nos permite utilizar los métodos y atributos de la clase
        ...
        self.delta_x = math.cos(math.radians(
            self.angulo))*constantes.VELOCIDAD_BALA
        self.delta_y = -math.sin(math.radians(self.angulo)) * \
            constantes.VELOCIDAD_BALA
